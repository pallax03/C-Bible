...
//provo a fare un vettore dinamico di insegnamenti
...
int main()
{
    ...
    */
    ...
    printf("\nSettimana 8 - Esercizio 2\t E-mail di riferimento: alex.mazzoni3@studio.unibo.it\n\n");
    
    
    //mi sono basato su questo esempio che ho fatto per creare un vettore dinamico, l'esempio e' perfettamente funzionante
    /*
        char **stringhe;
        ...
        printf("\n\n%c",stringhe[1][0]);    
    */
    //basandosi su questo codice a me basterebbe la parte di codice dinamica per *stringa per creare un vettore dinamico di strutture.
    //infatti facendo in questa maniera effettivamente memorizza i dati e li stampa, ma il programma crasha subito dopo aver chiesto lo scanf del menu non stampando nemmeno il printf seguente
    //chiedo aiuto o suggerimenti per questo problema.
    ...
    while (scelta!=0) { //solito menu che richiede una voce in input e finche' non si inserisce 0 si ripete
        ...
        scanf("%d", &scelta);
        fflush(stdin);
        printf("%d", scelta); //il programma crasha non mostrando nemmeno questa funzione. 
        switch (scelta) {
            case 0:
                printf("\nArrivederci.");
                break;
...
void aggiungiInsegnamento(char *cod, char *des, unsigned int anno, unsigned int cr) { //passo come argomento le variabili per riempire un insegnamento
    if(count_insegnamenti==0) 
        insegnamenti = (struct insegnamento *) malloc(sizeof(struct insegnamento)); 
    else
    {
        insegnamenti = (struct insegnamento *) realloc(insegnamenti, sizeof(struct insegnamento)); 
    }

    //lo alloco in memoria dinamicamente
    //if(insegnamenti != NULL)// se la malloc non riesce a aggiungere spazio in memoria 
    //{   
